...
Xvfb :1 -screen 0 1440x810x24 > /tmp/xvfb.log 2>&1 &

# Point DISPLAY to virtual X Server
export DISPLAY=:1

# Start a VNC server and point it to the same display
x11vnc -display :1 -quiet -nopw -rfbport 5901 -bg -o /tmp/vnc.log
...
emulator -avd ${ANDROID_PLATFORM}-emu -gpu host -no-audio -no-boot-anim -nojni -netfast -qemu -enable-kvm
